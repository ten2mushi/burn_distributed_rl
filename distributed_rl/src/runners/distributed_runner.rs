//! Distributed training runner that coordinates actors and learner.
//!
//! The DistributedRunner is the main entry point for distributed RL training.
//! It manages:
...
#[derive(Debug, Clone)]
pub struct DistributedRunnerConfig {
...
impl Default for DistributedRunnerConfig {
    fn default() -> Self {
        Self {
            n_actors: 4,
            n_envs_per_actor: 64,
...
impl DistributedRunner {
    ...
    pub fn new(config: DistributedRunnerConfig) -> Self {
        Self {
            config,
    ...
    /// Get configuration.
    pub fn config(&self) -> &DistributedRunnerConfig {
        &self.config
    }
...
/// Builder for DistributedRunner.
pub struct DistributedRunnerBuilder {
    config: DistributedRunnerConfig,
}

impl DistributedRunnerBuilder {
    /// Create a new builder with default config.
    pub fn new() -> Self {
        Self {
            config: DistributedRunnerConfig::default(),
        }
    }
    ...
    /// Build the runner.
    pub fn build(self) -> DistributedRunner {
        DistributedRunner::new(self.config)
    }
}

impl Default for DistributedRunnerBuilder {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    fn test_distributed_runner_config() {
        let config = DistributedRunnerConfig {
            n_actors: 8,
            n_envs_per_actor: 32,
    ...
    fn test_distributed_runner_builder() {
        let runner = DistributedRunnerBuilder::new()
            .n_actors(4)
            .n_envs_per_actor(64)
    ...
    fn test_shutdown_flag() {
        let runner = DistributedRunner::new(DistributedRunnerConfig::default());
