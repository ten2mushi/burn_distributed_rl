//! Distributed PPO algorithm implementation.
//!
...
use super::ppo_buffer::{PPORolloutBuffer, PPORolloutBufferConfig, PPORolloutBatch};
use crate::algorithms::algorithm::LossOutput;
use crate::algorithms::distributed_algorithm::{
    DistributedAlgorithm, DistributedPPOConfig, OnPolicyDistributed,
};
...
impl<B: AutodiffBackend> DistributedAlgorithm<B> for DistributedPPO {
    type Config = DistributedPPOConfig;
...
// Mark as on-policy
impl<B: AutodiffBackend> OnPolicyDistributed<B> for DistributedPPO {}
...
mod tests {
    ...
    fn test_distributed_ppo_new() {
        ...
        assert_eq!(<DistributedPPO as DistributedAlgorithm<B>>::name(&ppo), "DistributedPPO");
        assert!(!<DistributedPPO as DistributedAlgorithm<B>>::is_off_policy(&ppo));
        assert_eq!(<DistributedPPO as DistributedAlgorithm<B>>::n_epochs(&ppo), 4);
        assert_eq!(<DistributedPPO as DistributedAlgorithm<B>>::n_minibatches(&ppo), 4);
    }
    ...
    fn test_create_buffer() {
        ...
        let buffer = <DistributedPPO as DistributedAlgorithm<B>>::create_buffer(&ppo, 2, 32);
