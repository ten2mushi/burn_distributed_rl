...
///
/// # Implementations
/// - `DistributedPPO`: On-policy with GAE, consumes rollouts entirely
/// - `DistributedIMPALA`: Off-policy with V-trace correction
...
impl DistributedPPOConfig {
    ...
    ///
    /// ```rust
    /// use distributed_rl::algorithms::distributed_algorithm::DistributedPPOConfig;
    ///
    /// let config = DistributedPPOConfig::new().with_gamma(0.99);
...
pub trait OnPolicyDistributed<B: AutodiffBackend>: DistributedAlgorithm<B>
where
...
pub trait OffPolicyDistributed<B: AutodiffBackend>: DistributedAlgorithm<B>
where
